...
Finally, declare which ore should show up in the trace by writing an entry into the data map at `data/createreautomatedtraces/data_maps/block/ore_for_node.json`. The format is one entry per Node block id, each mapping to a single ore block. If the entry is missing the mod logs a warning once for that Node id and moves on without placing anything.
